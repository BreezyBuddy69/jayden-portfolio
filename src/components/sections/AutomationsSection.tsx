...
import { Bot, Workflow, Terminal, Zap, ExternalLink } from 'lucide-react'
...
export function AutomationsSection({ isActive, onOpenChat, language }: AutomationsSectionProps) {
  ...
  const SKILLS = [
    { icon: Bot,      title: t.aiAgentsTitle,  desc: t.aiAgentsDesc },
    { icon: Workflow, title: t.n8nTitle,        desc: t.n8nDesc },
    { icon: Terminal, title: t.claudeTitle,     desc: t.claudeDesc },
  ]
  ...
  return (
    ...
    >
      ...
      {/* Ambient */}
      <div aria-hidden className="absolute bottom-0 right-0 w-[50vw] h-[50vh] pointer-events-none"
        ...
        <div className="flex flex-col md:flex-row gap-8 mb-6">
          ...
          <div className="flex flex-col justify-center gap-4 flex-1">
            ...
            <motion.button
              ...
              onClick={onOpenChat}
              data-cursor="hover"
              className="flex items-center gap-3 px-4 py-3 rounded-xl w-fit group"
              style={{
            ...
            >
              <Zap className="w-4 h-4 flex-shrink-0" style={{ color: 'rgba(245,158,11,0.80)' }} />
              <div className="text-left">
                <p className="text-white/80 text-xs font-semibold">{t.ctaTitle}</p>
                <p className="text-white/38 text-[10px]">{t.ctaSub}</p>
              </div>
            </motion.button>

            {/* halovisionai.cloud link */}
            <motion.a
              href={HALO_URL}
              target="_blank"
              ...
              className="inline-flex items-center gap-1.5 text-[10px] font-medium group w-fit"
              style={{ color: 'rgba(245,158,11,0.50)', textDecoration: 'none' }}
            >
              <span className="group-hover:text-amber-400 transition-colors tracking-wide">halovisionai.cloud</span>
          ...
          {SKILLS.map((s, i) => {
            ...
            return (
              <motion.div
                ...
                className="flex-1 rounded-xl px-3 py-3 flex flex-col gap-1.5"
                style={{
              ...
              >
                <div className="flex items-center gap-2">
                  <Icon className="w-3 h-3 flex-shrink-0" style={{ color: 'rgba(245,158,11,0.65)' }} />
                  <p className="text-white/65 text-xs font-medium">{s.title}</p>
                </div>
                <p className="text-white/32 text-[10px] leading-relaxed">{s.desc}</p>
              </motion.div>
            )
          })}
        </div>
